...
| 代码基线 | `app/main.py`、`frontend/src`、`scripts/build_campus_dataset.py`、`scripts/import_campus_data.py`、`tests/test_app.py` |
| 覆盖范围 | 实施方案第 8 节全部四个开发阶段 |
| 文档状态 | 已完成本轮实现，待课程评审 |

> 本文档是实施方案的细化：实施方案回答"做什么、为什么"，本文档回答"具体怎么做"——表结构、接口契约、算法伪代码、模块划分、部署与测试设计。凡与现有原型不一致之处，均以第 15 节差距清单标注。
> 本轮实现已将 PDF 中的 290 条校园信息整理为独立结构化内容，支持推荐卡片查看全文、来源链接和基于正文的 RAG 问答；当前实现细节以代码和第 15 节差距清单为准。
...
现有实现已完成学生侧核心链路，并新增 PDF 结构化导入、内容全文详情接口和 RAG 多文档回答；本文档在其 API 路径与表名基础上向前设计，不推倒重来。
