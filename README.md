...
- 校园问答：TF-IDF/关键词融合、本地摘录、索引缓存、拒答与来源复查；可选 Jev 意图路由与证据检查。
...
后端读取用户环境变量 `TYPESAFE_API_KEY`；密钥不放入前端或仓库。在本机 `.env` 设置 `JEV_ENABLED=true` 后重启后端，进入“校园问答”即可使用自动分流和证据检查。默认示例配置关闭 Jev；无密钥或 API 失败时回退本地查询，并标注证据未检查。

启用后将问题和当前用户有权查看的最多 5 条资料摘录发送给 TypeSafe/Jev，不主动附带账号或学业档案。详情、阈值及虚构数据 live test 见 [Jev 接入与验证](docs/Jev接入与验证.md)。
...
本轮本地结果：84 项测试通过，前端构建通过。GitHub Actions 配置会运行相同检查。推荐评估样例为合成数据，仅用于验证流程；真实效果与论文结论需要独立标注数据。算法细节见 [第三阶段说明](docs/第三阶段_推荐算法优化与验证.md)。
